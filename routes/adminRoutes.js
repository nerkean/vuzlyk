...
router.get('/categories', csrfProtection, async (req, res, next) => {
    try {
        const categories = await Category.find().sort('order').lean();
        res.render('admin/categories', {
            pageTitle: 'Категорії - Адмін-панель',
            pageName: 'categories',
...
router.post('/categories', csrfProtection, async (req, res, next) => {
    try {
        ...
        res.redirect('/admin/categories?error=creation_failed');
    }
});

router.post('/categories/:id/delete', csrfProtection, async (req, res, next) => {
    try {
        await Category.findByIdAndDelete(req.params.id);
        res.redirect('/admin/categories');
    } catch (error) {
        console.error('Помилка видалення категорії:', error);
        res.redirect('/admin/categories?error=deletion_failed');
    }
});
...
router.get('/products/new', checkAdminAuth,  csrfProtection, async (req, res) => {
    const categories = await Category.find().sort('order').lean();
    ...
    sku 
        ...
        if (livePhotoReceived) {
            ...
            try {
                ...
                if (livePhotoReceived.path) try { await fs.unlink(livePhotoReceived.path); } catch (e) { }
            }
